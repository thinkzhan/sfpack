...
module.exports = (filename, dependencies) => {
    ...
    const p = isWebComp
        ...
        : path.resolve(filename, "index.html");
        ...
        const parser = new htmlparser.Parser(
            {
                ...
                onclosetag(name, attribs) {
                    if (isWebComp) {
                        switch (name) {
                            case "template":
                                code = originCode.substring(
                                    webCompStore.templateStart,
                                    parser.startIndex
                                );
                                break;
                            case "script":
                                webCompScript = originCode.substring(
                                    webCompStore.scriptStart,
                                    parser.startIndex
                                );
                                break;
                            case "style":
                                webCompStyle = originCode.substring(
                                    webCompStore.styleStart,
                                    parser.startIndex
                                );
                                break;
                        }
